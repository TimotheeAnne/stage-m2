...
def compute_samples(data,norm):
    ...
    for j in range(len(true_traj)):
        for t in range(50):
            inputs = np.concatenate((true_traj[j][t][:5],Acs[j][t]))
            targets = true_traj[j][t+1][:5] - true_traj[j][t][:5]
            Inputs.append(inputs)
            Targets.append(targets)
            if target[4] > 0.1:
                grasped_object += 1
    print( 'grasped_object transtions', grasped_object, grasped_object/(50*j))
    norm.init(Inputs,Targets)
    # ~ norm.pretty_print()
