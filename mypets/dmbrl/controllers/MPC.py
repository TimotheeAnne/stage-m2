...
class MPC(Controller):
    ...
    def __init__(self, params, window_size):
        ...
        ### Me
        if isinstance(params.env.observation_space, gym.spaces.dict_space.Dict):
            self.dO, self.dU = params.env.observation_space.spaces['observation'].shape[0], params.env.action_space.shape[0]
        else:
            self.goal_dim = None
            self.dO, self.dU = params.env.observation_space.shape[0], params.env.action_space.shape[0]
            
        self.dO, self.dU = 25, 4
        # ~ self.dO, self.dU = 8, 4
        self.best_val = None
        ...
        # ~ if self.model.is_tf_model:
            ...
            # ~ self.pred_cost, self.pred_traj = self._compile_cost(self.ac_seq, tf.zeros([1,self.optimizer.goal_dim], tf.float32) , get_pred_trajs=True)
            # ~ self.optimizer.setup(self._compile_cost, self._compile_cost_2, True)
    ...
    def train(self, obs_trajs, acs_trajs, rews_trajs):
        ...
        for obs, acs in zip(obs_trajs, acs_trajs):
            ...
            # ~ print( new_train_targs[0])
            # ~ print( new_train_in[0])
    ...
    def act(self, obs, t, goal_pos, cost_choice, get_pred_cost=False):
        ...
        if not self.has_been_trained:
            return np.random.uniform(self.ac_lb, self.ac_ub, self.ac_lb.shape), -1
        if self.ac_buf.shape[0] > 0:
            action, self.ac_buf = self.ac_buf[0], self.ac_buf[1:]
            return action, self.best_val

        if self.model.is_tf_model:
            self.sy_cur_obs.load(obs, self.model.sess)
        soln, self.best_val = self.optimizer.obtain_solution(self.prev_sol, self.init_var, goal_pos, cost_choice)
        ...
        if get_pred_cost and not (self.log_traj_preds or self.log_particles):
            ...
            if self.log_particles:
                ...
                self.pred_means.append(np.mean(pred_traj, axis=1, keepdims=True))
                self.pred_vars.append(np.mean(np.square(pred_traj - self.pred_means[-1]), axis=1))
            if get_pred_cost:
                return self.act(obs, t), pred_cost
        return self.act(obs, t, goal_pos, cost_choice)
    ...
    def _compile_cost(self, ac_seqs, goal_pos, get_pred_trajs=False):
        t, nopt, a = tf.constant(0), tf.shape(ac_seqs)[0], goal_pos
        ...
        if get_pred_trajs:
            ...
            def iteration(t, total_cost, cur_obs, pred_trajs):
                ...
                delta_cost = tf.reshape(
                    self.obs_cost_fn(next_obs, goal_pos) + self.ac_cost_fn(cur_acs), [-1, self.npart]
                )
            ...
            def iteration(t, total_cost, cur_obs,a):
                ...
                return t + 1, total_cost + delta_cost, self.obs_postproc2(next_obs),a

            _, costs, _ , _= tf.while_loop(
                cond=continue_prediction, body=iteration, loop_vars=[t, init_costs, init_obs,a]
            )
...
### Me
    def _compile_cost_2(self, ac_seqs, goal_pos, get_pred_trajs=False):
        t, nopt, a = tf.constant(0), tf.shape(ac_seqs)[0], goal_pos
        init_costs = tf.zeros([nopt])
        ac_seqs = tf.reshape(ac_seqs, [-1, self.plan_hor, self.dU])
        ac_seqs = tf.reshape(tf.tile(
            tf.transpose(ac_seqs, [1, 0, 2])[:, :, None],
            [1, 1, 5, 1]
        ), [self.plan_hor, -1, self.dU])
        init_obs = tf.tile(self.sy_cur_obs[None], [nopt * 5, 1])
        ...
        def iteration(t, total_cost, cur_obs,a):
            cur_acs = ac_seqs[t]
            next_obs, pred = self._predict_next_obs_2(cur_obs, cur_acs)

            score = []
            for ac in range(500):
                _, var = tf.nn.moments(pred[:,ac], axes=0)
                score.append(tf.reduce_sum( var ) )
                
            delta_cost = tf.convert_to_tensor(score) 
            return t + 1, total_cost + delta_cost, self.obs_postproc2(next_obs),a
    ...
    def _predict_next_obs_2(self, obs, acs):
        proc_obs =  self.obs_preproc(obs)

        if self.model.is_tf_model:
            if self.prop_mode == "TS1" or self.prop_mode == "TSinf":
                proc_obs, acs = self._expand_to_ts_format_2(proc_obs), self._expand_to_ts_format_2(acs)
            
            # Obtain model predictions
            inputs = tf.concat([proc_obs, acs], axis=-1)
            mean, _ = self.model.create_prediction_tensors(inputs)
            predictions = mean

            # TS Optimization: Remove additional dimension
            if self.prop_mode == "TS1" or self.prop_mode == "TSinf":
                flatten_predictions = self._flatten_to_matrix_2(predictions)
            
            return self.obs_postproc(obs, flatten_predictions), predictions
        else:
            raise NotImplementedError()
    
    def _expand_to_ts_format_2(self, mat):
        dim = mat.get_shape()[-1]
        return tf.reshape(
            tf.transpose(
                tf.reshape(mat, [-1, self.model.num_nets, 5 // self.model.num_nets, dim]),
                [1, 0, 2, 3]
            ),
            [self.model.num_nets, -1, dim]
        )

    def _flatten_to_matrix_2(self, ts_fmt_arr):
        dim = ts_fmt_arr.get_shape()[-1]
        return tf.reshape(
            tf.transpose(
                tf.reshape(ts_fmt_arr, [self.model.num_nets, -1, 5 // self.model.num_nets, dim]),
                [1, 0, 2, 3]
            ),
            [-1, dim]
        )
