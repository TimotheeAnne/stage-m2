...
class FC:
    ...
    def compute_output_tensor(self, input_tensor):
        ...
        # Me 
        # ~ first_half  = FC._activations[self.activation](raw_output[:,:,:25])
        # ~ boolean = FC._activations['sigmoid']( raw_output[:,:,25:26])
        # ~ second_half = FC._activations[self.activation](raw_output[:,:,26:])
        # ~ return tf.concat((first_half, boolean, second_half),axis=2)
