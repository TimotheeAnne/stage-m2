...
def get_outcome(states,  nb_traj_steps):
    #print(len(states))
...
def get_state(state):
    s = list(state[3:6]) + list(state[8:10]) + list(state[12:18])
    return np.array(s)
...
def run_episode(model_type, model, policy_params, explo_noise, nb_traj_steps, size_sequential_nn, 
                Actions, Observations, max_step=50, focus_range=None, add_noise=False):
    obs, acs = [], []
    out = env.reset()
    obs.append(out.copy())
    state = get_state(out)
    ...
    return get_outcome(states, nb_traj_steps), states
...
for i in tqdm(range(starting_iteration, max_iterations)):
    ...
    outcome, states = run_episode(model_type, param_policy, policy_params, exploration_noise, nb_traj_steps,
                                  size_sequential_nn, focus_range=focus, add_noise=add_noise, 
...
with open('/home/tim/Documents/stage-m2/tf_test/data/imgep.pk','bw') as f:
