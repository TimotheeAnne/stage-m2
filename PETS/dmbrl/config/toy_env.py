...
class ToyEnvConfigModule:
    ...
    def obs_cost_fn(obs):
        ...
        if isinstance(obs, np.ndarray):
            return np.map(lambda o: o if o>=0 else -0.5*o, obs)
        else:
            return tf.map_fn(lambda o:tf.cond(o[0] >=0, lambda: o[0], lambda: -0.5*o[0] ),obs)
    ...
    def nn_constructor(self, model_init_cfg):
        ...
        if not model_init_cfg.get("load_model", False):
            model.add(FC(20, input_dim=self.MODEL_IN, activation='swish', weight_decay=0.0001)) #500
            model.add(FC(20, activation='swish', weight_decay=0.00025))#500
            model.add(FC(20, activation='swish', weight_decay=0.00025))#500
